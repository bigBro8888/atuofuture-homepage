      ...
      items: [
        {
          ...
          icon: 'groups',
          tags: [
            { label: '', icon: 'touch_app', logoImage: '' },
            { label: '', icon: 'link', logoImage: '' },
            { label: '', icon: 'restart_alt', logoImage: '' },
          ],
        },
        {
          title: '',
          ...
          icon: 'apartment',
          tags: [
            { label: '', icon: 'eco', logoImage: '' },
            { label: '', icon: 'deployed_code', logoImage: '' },
            { label: '', icon: 'monitoring', logoImage: '' },
          ],
        },
        {
          title: '',
          ...
          icon: 'diamond',
          tags: [
            { label: '', icon: 'login', logoImage: '' },
            { label: '', icon: 'play_circle', logoImage: '' },
            { label: '', icon: 'desktop_windows', logoImage: '' },
          ],
        },
      ],
    },
...
function cleanSceneTag(value = {}, fallback = {}) {
  if (typeof value === 'string') {
    return {
      label: cleanText(value, typeof fallback === 'string' ? fallback : fallback.label || '', 20),
      icon: cleanText(typeof fallback === 'object' ? fallback.icon : '', '', 40) || 'circle',
      logoImage: cleanUrl(typeof fallback === 'object' ? fallback.logoImage : undefined, ''),
    }
  }
  const fb = typeof fallback === 'object' && fallback ? fallback : {}
  return {
    label: cleanText(value.label, fb.label || '', 20),
    icon: cleanText(value.icon, fb.icon || 'circle', 40) || 'circle',
    logoImage: cleanUrl(value.logoImage !== undefined ? value.logoImage : undefined, fb.logoImage || ''),
  }
}

function cleanScene(value = {}, fallback = {}) {
  const hasTags = Array.isArray(value.tags)
  const tags = Array.from({ length: 3 }, (_, index) => {
    const raw = hasTags ? value.tags[index] : undefined
    const fb = Array.isArray(fallback.tags) ? fallback.tags[index] : undefined
    return cleanSceneTag(raw === undefined ? {} : raw, fb || {})
  })
  return {
    title: cleanText(value.title, fallback.title || '', 40),
    subtitle: cleanText(value.subtitle, fallback.subtitle || '', 60),
    desc: cleanText(value.desc, fallback.desc || '', 280),
    sceneImage: cleanUrl(value.sceneImage, fallback.sceneImage || ''),
    logoImage: cleanUrl(value.logoImage !== undefined ? value.logoImage : undefined, fallback.logoImage || ''),
    icon: cleanText(value.icon, fallback.icon || 'category', 40) || 'category',
    tags,
  }
}
...
function cleanAlbumItem(value = {}, fallback = {}) {
  return {
...
function cleanAlbum(value = {}, fallback = {}) {
  ...
  return {
    title: cleanText(value.title, fallback.title || '产品相册', 80),
    items,
  }
}
...
function cleanStory(value = {}, fallback = {}) {
  ...
  const scenes = padList(scenesIn.items, 3, () => ({
    title: '',
    subtitle: '',
    desc: '',
    sceneImage: '',
